  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg-primary)] pt-24 sm:pt-32 pb-16 sm:pb-24">
      <div className="max-w-7xl mx-auto px-5 sm:px-6">
        ...
        {/* ── 1. HEADER ───────────────────────────────────────── */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 sm:mb-16 animate-fade-up border-b border-[var(--color-border)] pb-10 sm:pb-12 gap-8">

          {/* Title + subtitle */}
          <div className="max-w-2xl">
            <h1 className="text-[2.6rem] leading-[1.05] sm:text-6xl md:text-7xl font-medium tracking-tighter text-[var(--color-text-primary)] mb-3 sm:mb-4">
              Insights &{" "}
          ...
          <div className="w-full md:w-auto shrink-0">
            <form className="flex w-full md:w-[300px] lg:w-[320px] relative">
              <input
                type="email"
                placeholder="Get updates in your inbox"
                className="w-full bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-full py-3 pl-5 pr-28 text-sm font-medium tracking-tight text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--color-dark)] transition-all"
                required
              />
              <button
                type="submit"
                className="absolute right-1 top-1 bottom-1 bg-[var(--color-dark)] text-white px-4 sm:px-5 rounded-full text-xs font-medium hover:bg-opacity-90 transition-all"
              >
                Subscribe
              </button>
            </form>
        ...
        {featuredPost && (
          ...
          >
            <div className="flex flex-col lg:flex-row bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-[1.5rem] sm:rounded-[2rem] overflow-hidden hover:shadow-lg transition-all duration-300">

              {/* Thumbnail */}
              <div className="w-full lg:w-1/2 h-52 sm:h-64 lg:h-auto bg-slate-200 relative overflow-hidden shrink-0">
                <div className="absolute inset-0 bg-gradient-to-tr from-slate-300 to-slate-100 group-hover:scale-105 transition-transform duration-700" />
              ...
              <div className="w-full lg:w-1/2 p-7 sm:p-10 md:p-12 flex flex-col justify-center">
                <div className="flex items-center gap-3 mb-4 sm:mb-6">
                  <span className="px-3 py-1 bg-white border border-[var(--color-border)] rounded-full text-[10px] sm:text-xs font-semibold uppercase tracking-widest text-[var(--color-text-secondary)]">
                ...
                <h2 className="text-2xl sm:text-3xl md:text-4xl font-medium tracking-tight text-[var(--color-text-primary)] mb-3 sm:mb-4 group-hover:text-[var(--color-dark)]/70 transition-colors leading-[1.15]">
                  {featuredPost.title}
                </h2>
                <p className="text-sm sm:text-base text-[var(--color-text-secondary)] font-light leading-relaxed mb-6 sm:mb-8">
                  {featuredPost.excerpt}
                </p>
                <div className="flex items-center gap-2 text-sm font-medium text-[var(--color-dark)] mt-auto">
                  Read Article
                  <svg
                    width="15"
                    height="15"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 animate-fade-up delay-300">
            {regularPosts.map((post) => (
              <Link
                to={`/blog/${post.id}`}
                key={post.id}
                className="group flex flex-col bg-white border border-[var(--color-border)] rounded-[1.25rem] sm:rounded-[1.5rem] overflow-hidden hover:shadow-md hover:border-[var(--color-dark)]/20 transition-all duration-300"
              >
                {/* Thumbnail */}
                <div className="w-full h-44 sm:h-48 bg-slate-100 relative overflow-hidden shrink-0">
                  <div className="absolute inset-0 bg-gradient-to-tr from-slate-200 to-slate-50 group-hover:scale-105 transition-transform duration-700" />
                </div>
                ...
                <div className="p-5 sm:p-6 md:p-8 flex flex-col flex-grow">
                  ...
                  <h3 className="text-base sm:text-lg md:text-xl font-medium tracking-tight text-[var(--color-text-primary)] mb-2 sm:mb-3 group-hover:text-[var(--color-dark)]/70 transition-colors leading-snug">
                    {post.title}
                  </h3>

                  <p className="text-[var(--color-text-secondary)] text-sm font-light leading-relaxed mb-5 sm:mb-6 flex-grow">
                    {post.excerpt}
                  </p>

                  <div className="flex items-center gap-2 text-sm font-medium text-[var(--color-text-primary)] mt-auto">
                    Read More
                    <svg
                      width="13"
                      height="13"
        ...
        {filteredPosts.length === 0 && (
          <div className="text-center py-20 sm:py-32 border border-dashed border-[var(--color-border)] rounded-[1.5rem] sm:rounded-[2rem] bg-[var(--color-bg-secondary)]">
        ...
        {/* ── 5. BOTTOM CTA ───────────────────────────────────── */}
        <div className="mt-20 sm:mt-32 pt-12 sm:pt-16 border-t border-[var(--color-border)] text-center animate-fade-up delay-400">
